...
def train(config, args, datasets_name):
    ...
    for dataset_name in datasets_name:
        ...
        for repeat in range(args.repeat_num):
            ...
            model = GCN(input_dim=input_dim, hidden_dim=config['hidden_dim'], output_dim=output_dim,
            ...
            for epoch in range(config['epoch_num']):
                ...
                for batch in dataloader:
                    ...
                    loss_train += loss_func(out[batch.train_mask], batch.y[batch.train_mask]).cpu().data.numpy()
                ...
                if epoch % args.epoch_log == 0:
                    ...
                    print("[Train] Expected: %s\nActual: %s" % (batch.y[batch.train_mask], out[batch.train_mask]))
                    loss_train += loss_func(out[batch.train_mask], batch.y[batch.train_mask]).cpu().data.numpy()
                    loss_val += loss_func(out[batch.val_mask], batch.y[batch.val_mask]).cpu().data.numpy()
                    loss_test += loss_func(out[batch.test_mask], batch.y[batch.test_mask]).cpu().data.numpy()
...
def main():
    ...
    if args.tune:
        for dataset_name in datasets_name:
            ...
            # NUM_SAMPLES = 5000
            NUM_SAMPLES = 1000
            result = tune.run(partial(train, args=args, datasets_name=[dataset_name]),
                              resources_per_trial={"cpu": 8, "gpu": 0},
                              config=GCN_CONFIG, num_samples=NUM_SAMPLES, scheduler=scheduler,
        ...
        }, args=args, datasets_name=datasets_name)
