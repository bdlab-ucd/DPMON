...
class GCN2(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, hidden_dim_name,
                 layer_num, alpha, theta, shared_weights=True, dropout=0.0, **kwargs):
        ...
        self.linear = torch.nn.ModuleList([tg.nn.Linear(input_dim, hidden_dim), tg.nn.Linear(hidden_dim, output_dim)])
        self.convs = nn.ModuleList([tg.nn.GCN2Conv(hidden_dim, alpha, theta, layer_num+1, shared_weights, normalize=False) for i in range(layer_num - 2)])
...
class GAT(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, hidden_dim_name,
        ...
        self.conv_hidden = nn.ModuleList([tg.nn.GATConv(hidden_dim, hidden_dim, edge_dim=1) for i in range(layer_num - 2)])
    ...
    def forward(self, data):
        ...
        t_np = x.cpu().data.numpy()  # convert to Numpy array
        df = pd.DataFrame(t_np)  # convert to a dataframe
        df.to_csv("HiddenLayerOutput_GAT_%s" % self.hidden_dim_name, index=False)  # save to file
...
class GAT2(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, hidden_dim_name,
        ...
        self.conv_first = tg.nn.GATv2Conv(input_dim, hidden_dim, heads=1, edge_dim=1, fill_value='mean')
        self.conv_hidden = nn.ModuleList([tg.nn.GATConv(hidden_dim, hidden_dim, heads=1, edge_dim=1, fill_value='mean') for i in range(layer_num - 2)])
        self.conv_out = tg.nn.GATv2Conv(hidden_dim, output_dim, heads=1, edge_dim=1, fill_value='mean')
...
class SAGE(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        t_np = x.cpu().data.numpy()  # convert to Numpy array
        df = pd.DataFrame(t_np)  # convert to a dataframe
        df.to_csv("HiddenLayerOutput_SAGE_%s" % self.hidden_dim_name, index=False)  # save to file
...
class GIN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, hidden_dim_name,
        ...
        self.conv_hidden_nn = nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for i in range(layer_num - 2)])
        self.conv_hidden = nn.ModuleList([tg.nn.GINEConv(self.conv_hidden_nn[i], edge_dim=1) for i in range(layer_num - 2)])
        self.conv_out_nn = nn.Linear(hidden_dim, output_dim)
    ...
    def forward(self, data):
        ...
        t_np = x.cpu().data.numpy()  # convert to Numpy array
        df = pd.DataFrame(t_np)  # convert to a dataframe
        df.to_csv("HiddenLayerOutput_GIN_%s" % self.hidden_dim_name, index=False)  # save to file
